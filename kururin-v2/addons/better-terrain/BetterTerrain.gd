...
## Custom function copied and edited from _update_tile_tiles(). probably inefficient.
## should only be called for decoration tiles, only returns a tile if all connections match.
## If multiple decors have all connections match, return the one with more connections.
## If both/all decors have the same number of connections (and all match) return weighted random.
func _update_tile_decoration(tm: TileMapLayer, coord: Vector2i, types: Dictionary, cache: Array, apply_empty_probability: bool):
	var type = types[coord]
	var best := []
	var highest_connection_count = -1;
	
	## NOTE: connections(at least for tile matching, not vertex matching) have ids as such
	## 11 12 15
	##  8  X  0
	##  7  4  3
	for t in cache[type]:
		var needed_matches: Dictionary[int, bool]
		for key in t[3]:
			needed_matches[key] = false
		
		for peering in t[3]:
			if t[3][peering].has(types[tm.get_neighbor_cell(coord, peering)]):
				needed_matches[peering] = true
			#score += 3 if t[3][peering].has(types[tm.get_neighbor_cell(coord, peering)]) else -10
		
		var all_connection_match = true
		
		## check if all connections match
		for key in needed_matches:
			if needed_matches[key] == false:
				#print("key not true, id:" + str(key))
				all_connection_match = false
				break
		
		if all_connection_match and (needed_matches.size() > highest_connection_count):
			highest_connection_count = needed_matches.size()
			best = [t]
		elif all_connection_match and (needed_matches.size() == highest_connection_count):
			best.append(t)
...
func _update_tile_immediate(tm: TileMapLayer, coord: Vector2i, ts_meta: Dictionary, types: Dictionary, cache: Array) -> void:
	...
	if terrain[2] ==  TerrainType.DECORATION:
		placement = _update_tile_decoration(tm, coord, types, cache, terrain[2] == TerrainType.DECORATION)
	elif terrain[2] in [TerrainType.MATCH_TILES, TerrainType.DECORATION]:
