...
# TODO: Maybe should be stored in the settings controller singleton
...
var reading_keybind_input: bool = false ## true after pressing a keybind button, prevents reading from 2 at the same time
var input_map_key: String ## contains key of action to put data in
var reading_keybind_button: Button 

# Called when the node enters the scene tree for the first time.
func _ready():
	## TODO BUG Something in here clears double mapped inputs (i.e. if controller and keyboard are mapped to the same input
	add_resolutions()
	connect_keybind_signals()
	init_values()
...
func _input(event: InputEvent):
	...
	## if we are currently reading input for a keybind
	if event is InputEventKey or \
			(event is InputEventMouseButton and event.button_mask != 0): # so it doesnt register upon clicking the button to activate. Can also check event.pressed == false if this doesnt work, not sure the difference
		print(event)
		# Save new binding in settings
		SettingsController.save_keybind_setting(input_map_key, event)
...
## Connect all the keybind buttons to this script, can do manually but it's a pain
func connect_keybind_signals():
	for keybind_button: KeybindButton in get_tree().get_nodes_in_group("KeybindingButton"):
		keybind_button.keybind_button_pressed.connect(_on_keybind_button_pressed)
...
func init_values():
	...
	for keybind_button: Button in get_tree().get_nodes_in_group("KeybindingButton"):
		...
		if InputMap.has_action(key):
			var action = SettingsController.config.get_value("keybindings", key)
			keybind_button.text = action.as_text()
			#keybind_button.text = action[0].as_text()
		else:
			keybind_button.text = "ERROR: couldn't read keybind"
	
	## Video settings
	var resolution: Vector2i = SettingsController.config.get_value("video", "resolution")
	%ResolutionDropdown.selected = resolutions.keys().find(str(resolution.x, "x", resolution.y))
	var fullscreen: bool =  SettingsController.config.get_value("video", "fullscreen")
	%FullscreenCheckButton.button_pressed = fullscreen
	
	## Audio settings
	%MasterVolumeSlider.set_value_no_signal(SettingsController.config.get_value("audio", "master_volume"))
	%MusicVolumeSlider.set_value_no_signal(SettingsController.config.get_value("audio", "music_volume"))
	%SFXVolumeSlider.set_value_no_signal(SettingsController.config.get_value("audio", "sfx_volume"))


func _on_master_volume_slider_value_changed(value):
	AudioServer.set_bus_volume_db(AudioServer.get_bus_index("Master"), linear_to_db(value))
	SettingsController.save_audio_setting("master_volume", value)


func _on_music_volume_slider_value_changed(value):
	AudioServer.set_bus_volume_db(AudioServer.get_bus_index("Music"), linear_to_db(value))
	SettingsController.save_audio_setting("music_volume", value)


func _on_sfx_volume_slider_value_changed(value):
	AudioServer.set_bus_volume_db(AudioServer.get_bus_index("SFX"), linear_to_db(value))
	SettingsController.save_audio_setting("sfx_volume", value)
...
func _on_resolution_dropdown_item_selected(index):
	...
	SettingsController.save_video_setting("resolution", resolutions[key])
...
func _on_fullscreen_check_button_toggled(toggled_on):
	if toggled_on:
		...
		%ResolutionDropdown.selected = resolutions.keys().find(str(resolution.x, "x", resolution.y))
	SettingsController.save_video_setting("fullscreen", toggled_on)
